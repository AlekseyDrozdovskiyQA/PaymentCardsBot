...
bot = TeleBot(token='5473663275:AAFkcvevr4_aQXebWGn2TJkeRkDly4owBP4', parse_mode='html') # создание бота
...
def start_command_handler(message: types.Message):
    ...
    bot.send_message(
        ...
        text='Привет! Я генерирую номер тестовой карты\nВыбери нужный тип карты:', # текст сообщения
...
def message_handler(message: types.Message):
    ...
    card_code = faker.credit_card_security_code(card_type)
    card_date = faker.credit_card_expire()
    # и выводим пользователю
    bot.send_message(
        chat_id=message.chat.id,
        text=f'Тестовая карта {card_type}:\nНомер карты: <code>{card_number}</code>\nКод карты: <code>{card_code}</code>\nСрок карты: <code>{card_date}</code>'
    )
